/**
 * Demo: the accent-color picker, nested under Settings.
 *
 * Exercises html descriptions (the swatches), a per-search searchDelay, and
 * a per-search selectCallback.
 */
...
export default {
    breadcrumbText: 'Accent color',
    text: 'Accent color',
    trackerId: 'accent-color',
    description: 'Change the workspace theme',
    searchDelay: 100,

    searchCallback(searchText, resultHandler) {
        const results = COLORS.filter((color) => {
            return resultHandler.filters.isMatch(searchText, `${color.hex} ${color.name}`);
        }).map((color) => ({
            ...color,
            trackerId: color.name,
            html: color.name,
            description: {
                html: `<span class="demo-swatch" style="background-color: #${color.hex}"></span>`
            }
        }));
    ...
    selectCallback(selected) {
        showSelection(`Accent color set to ${selected.selectedValue.name}`);
    }
};
