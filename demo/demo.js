...
const mainSwitcher = lstrQuickSwitcher({
    ...
    searchCallback(searchText, resultHandler) {
        ...
        const results = [
            ...
            ...SECTIONS
                .filter((section) => matches(`${section.text} ${section.description}`))
                .map((section) => ({...section, trackerStaticSort: 0})),
...
const simpleSwitcher = lstrQuickSwitcher({
    ...
    searchCallback(searchText, resultHandler) {
        ...
        resultHandler.setResults(
            options.filter((item) => resultHandler.filters.isMatch(searchText, item))
        );
    },
...
document.querySelector('#open-main')
    .addEventListener('click', () => mainSwitcher.open());

document.querySelector('#open-simple')
    .addEventListener('click', () => simpleSwitcher.open());
